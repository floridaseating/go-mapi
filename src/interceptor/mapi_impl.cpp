...
ULONG MapiImpl::MAPILogon(
    ...
    *lphSession = 1;  // Stable process-local session for this compose provider.
...
ULONG MapiImpl::MAPIFreeBuffer(LPVOID pv) {
    // None of the currently supported calls return provider-owned buffers.
    // Keep this a successful no-op until ResolveName is implemented and its
    // ownership is proven through the native Windows MAPI32 stub.
    (void)pv;
    return SUCCESS_SUCCESS;
}
...
ULONG MapiImpl::MAPIResolveName(
    ...
    // Export the SDK surface without inventing allocator semantics. A real
    // recipient allocation follows only after a through-MAPI32 x86/x64 test.
    return MAPI_E_NOT_SUPPORTED;
}
...
ULONG MapiImpl::MAPISendDocuments(
    ...
    // Never claim success while silently discarding the requested documents.
    return MAPI_E_NOT_SUPPORTED;
}
